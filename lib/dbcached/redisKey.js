...
function getRedisKey(model, type, where_data, sort_data) {
  ...
  if (type == 'd') {
    ...
    if (where_data) {
      ...
      r_key += (0, _yhqs._memkey)(where_data, '');
    }
    r_key += SEPARATOR;
    if (sort_data) {
      // r_key += JSON.stringify(sort_data);
      r_key += (0, _yhqs._memkey)(sort_data, '');
    }
  } else {
...
function parseRedisKey(r_key) {
  ...
  if (type == 'd') {
    ...
    if (result[2]) where = (0, _yhqs._memParseKey)(result[2], '');
    if (result[3]) {
      sort = (0, _yhqs._memParseKey)(result[3], '');
    }
  } else {
    debug('error! not support type:' + type);
  }
