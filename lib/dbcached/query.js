...
function itemFulfillQuery(item) {
  ...
  switch (op) {
    ...
    case '':
      {
        // 判断值是否等于条件中值
        var _attrValue4 = getItemAttrValue(item, attr);
        // debug('itemFulfillQuery:', JSON.stringify({op, attr, attrValue}), type(query));

        if ((0, _type2.default)(query) == 'array') {
          // 需要判断attrValue是否在数组中.
          for (var _i2 = 0; _i2 < query.length; _i2++) {
            if (query[_i2] == _attrValue4) return true;
          }
          return false;
        } else if ((0, _type2.default)(_attrValue4) == 'array') {
          for (var _i3 = 0; _i3 < _attrValue4.length; _i3++) {
            if (query == _attrValue4[_i3]) return true;
          }
          return false;
        } else {
          return _attrValue4 == query;
        }
      }
    default:
      {
